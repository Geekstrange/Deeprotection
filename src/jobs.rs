...
impl JobManager {
    ...
    pub fn reap_done(&mut self) {
        self.jobs.retain(|_, j| !matches!(j.status, JobStatus::Done(_)));
    }
    ...
    pub fn last_job(&self) -> Option<&Job> {
        self.jobs.values().rev().find(|j| {
            matches!(j.status, JobStatus::Stopped | JobStatus::Background)
        })
    }
    ...
    pub fn wait_foreground(&mut self, pgid: Pid, cmd_display: &str) {
        loop {
            ...
            match waitpid(Pid::from_raw(-pgid.as_raw()),
    ...
    pub fn poll_background(&mut self) {
        let pgids: Vec<Pid> = self.jobs.values()
