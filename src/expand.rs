...
impl std::fmt::Display for ExpandError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            ExpandError::TooManyArgs { limit, actual } =>
                write!(f, "glob expansion produced {} arguments (limit {})", actual, limit),
        }
    }
}
...
fn try_char_range(prefix: &str, inner: &str, suffix: &str) -> Option<Vec<String>> {
    let (lo_str, hi_str) = inner.split_once("..")?;
    let lo = lo_str.trim().chars().next().filter(|_| lo_str.trim().len() == 1)?;
    let hi = hi_str.trim().chars().next().filter(|_| hi_str.trim().len() == 1)?;
    ...
    Some(range.map(|c| format!("{}{}{}", prefix, c, suffix)).collect())
}
...
// ──────────────────────────────────────────────────────────────────────────────
// Tests (unchanged from original; new tests added at end)
// ──────────────────────────────────────────────────────────────────────────────
...
mod tests {
    ...
    fn in_temp_dir<F: FnOnce()>(f: F) {
        let dir = std::env::temp_dir().join(format!("dpshell_test_{}", std::process::id()));
    ...
    fn glob_char_detection() {
        assert!(contains_glob_char("*.log"));
        assert!(contains_glob_char("file?.txt"));
        assert!(contains_glob_char("[abc]def"));
        assert!(!contains_glob_char("normal.txt"));
        assert!(!contains_glob_char("-flag"));
        assert!(!contains_glob_char("--long-flag"));
    }

    #[test]
    fn no_match_returns_pattern() {
        in_temp_dir(|| {
            let argv = vec!["rm".to_string(), "*.DEAD".to_string()];
            let expanded = expand_argv(&argv).unwrap();
            assert_eq!(expanded, vec!["rm", "*.DEAD"]);
        });
    }

    // ── Regression test for integer-overflow fix ──────────────────────────────
    #[test]
    fn numeric_range_at_i64_extremes_does_not_panic() {
        // Previously panicked in debug due to (hi - lo) overflow.
        let r = expand_braces("file{0..9223372036854775807}");
        // Must NOT panic; must refuse to expand (returns the literal token).
        assert_eq!(r, vec!["file{0..9223372036854775807}"]);
    }

    #[test]
    fn numeric_range_within_limit_works() {
        let r = expand_braces("x{1..3}y");
        assert_eq!(r, vec!["x1y", "x2y", "x3y"]);
    }
