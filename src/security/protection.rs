...
use crate::parser::syntax::{flatten_commands, CommandNode, SimpleCommand};
use std::env;
...
pub fn check_node(
    ...
    let cwd = match env::current_dir() {
        Ok(p) => p,
        Err(e) => {
            return ProtectionResult::Blocked(format!("(cwd unavailable: {} — failing closed)", e));
        }
    };
...
pub fn check_expanded_argv(
    ...
    let path_args: Vec<&str> = expanded_argv
        .iter()
        .skip(1)
    ...
    let implicit_hit =
        !explicit_hit && path_args.is_empty() && is_under_protection(&cwd, protected_paths);
